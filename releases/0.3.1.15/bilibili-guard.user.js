...
(() => {
  ...
  function createPanel({
    ...
    section.append(header, status, update, actions, anchorSign, list);
    ...
    function renderAnchorSignResult(value) {
      ...
      if (item.starAnchor === 'yes') {
        businessFields.push(textElement(
          ...
          'div',
          'guard-anchor-sign-meta',
          `星级任务：${Number.isSafeInteger(item.starLevel) && item.starLevel >= 0 ? item.starLevel : '未知'}`,
        ));
      }
...
(() => {
  ...
  async function start(dependencies) {
    ...
    const apiFetch = runtime.userscriptHttp.createFetch({
      ...
      allowedHosts: ['api.bilibili.com', 'api.live.bilibili.com', 'api.gscn.live'],
    });
