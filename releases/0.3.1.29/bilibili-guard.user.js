...
(() => {
  ...
  const ENDPOINTS = Object.freeze({
    ...
    liverGuild: 'https://fetch.danmakus.com/https://workers.vrp.moe/laplace/bilibili-user/',
  });
  ...
  const MAX_LIVER_GUILD_BODY_BYTES = 64 * 1024;
  const LIVER_GUILD_TIMEOUT_MS = 4000;
  ...
  function emptyLiverGuild(status = 'unavailable') {
    return {status, name: ''};
  }

  function normalizeLiverGuildPayload(payload) {
    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
      return emptyLiverGuild();
    }
    const guildInfo = payload.guildInfo;
    if (guildInfo === null || guildInfo === undefined) return emptyLiverGuild('no_data');
    if (!guildInfo || typeof guildInfo !== 'object' || Array.isArray(guildInfo)) {
      return emptyLiverGuild();
    }
    const history = guildInfo.history;
    if (!Array.isArray(history)) return emptyLiverGuild();
    if (history.length === 0) return emptyLiverGuild('no_data');
    if (history.length > 1000) return emptyLiverGuild();
    const latest = history.at(-1);
    if (!latest || typeof latest !== 'object' || Array.isArray(latest)) return emptyLiverGuild();
    const name = typeof latest.name === 'string' ? latest.name.trim().slice(0, 120) : '';
    return name ? {status: 'found', name} : emptyLiverGuild('no_data');
  }
  ...
  function createBilibiliApi({fetchImpl, sleep, timeoutMs = 8000}) {
    ...
    async function fetchLiverGuild(uidValue) {
      const unavailable = Object.freeze(emptyLiverGuild());
      const uid = normalizeUid(uidValue);
      if (!uid) return unavailable;
      const url = `${ENDPOINTS.liverGuild}${uid}`;
      const controller = new AbortController();
      const timeout = setTimeout(
        () => controller.abort(),
        Math.min(timeoutMs, LIVER_GUILD_TIMEOUT_MS),
      );
      try {
        ...
        if (contentLength !== null && contentLength !== undefined) {
          const size = Number(contentLength);
          if (!Number.isFinite(size) || size < 0 || size > MAX_LIVER_GUILD_BODY_BYTES) {
            return unavailable;
          }
        }
        const text = await response.text();
        if (typeof text !== 'string' || text.length > MAX_LIVER_GUILD_BODY_BYTES) {
          return unavailable;
        }
        let payload;
        try {
        ...
        return Object.freeze(normalizeLiverGuildPayload(payload));
      } catch {
        return unavailable;
      } finally {
        clearTimeout(timeout);
...
(() => {
  ...
  const LIVER_GUILD_TTL_MS = 900_000;
  ...
  function createGuardBroker({api, storageArea, now}) {
    ...
    function normalizeStreamRevenue(value) {
      ...
      if (value.status === 'found') {
        const {total, giftSc, guard, charge} = value;
        if (
          [total, giftSc, guard, charge].every(number =>
            typeof number === 'number' && Number.isFinite(number) && number >= 0 && number <= 100_000_000,
          )
          && Math.abs((giftSc + guard + charge) - total) <= 0.000001
        ) {
          return {status: 'found', total, giftSc, guard, charge};
        }
      }
      if (value.status === 'no_data' || value.status === 'verification_required') {
        return {status: value.status, total: null, giftSc: null, guard: null, charge: null};
      }
      return {status: 'unavailable', total: null, giftSc: null, guard: null, charge: null};
    }

    function normalizeLiverGuild(value) {
      if (!value || typeof value !== 'object' || Array.isArray(value)) {
        return {status: 'unavailable', name: ''};
      }
      if (value.status === 'found' && typeof value.name === 'string') {
        const name = value.name.trim().slice(0, 120);
        if (name) return {status: 'found', name};
      }
      if (value.status === 'no_data') return {status: 'no_data', name: ''};
      return {status: 'unavailable', name: ''};
    }
    ...
    async function fetchStreamRevenue(uid, {force = false, includeVerificationCookies = false} = {}) {
      ...
      return coalesce(inFlight, `stream-revenue:${uid}:${requestKind}`, async () => {
        ...
        try {
          ...
          revenue = {status: 'unavailable', total: null, giftSc: null, guard: null, charge: null};
        }
    ...
    async function fetchLiverGuild(uid) {
      const key = `liverGuild:${uid}`;
      const cached = await sessionCache.read(key);
      if (cached.state === 'fresh') return cached.value;
      return coalesce(inFlight, `liver-guild:${uid}`, async () => {
        const shared = await sessionCache.read(key);
        if (shared.state === 'fresh') return shared.value;
        let guild;
        try {
          guild = normalizeLiverGuild(await api.fetchLiverGuild(uid));
        } catch {
          guild = {status: 'unavailable', name: ''};
        }
        await sessionCache.write(key, guild, LIVER_GUILD_TTL_MS);
        return guild;
      });
    }
    ...
    function summaryValue(creator, firstPage, ipLocation, streamRevenue, liverGuild, fetchedAt) {
      return {
        creator,
        total: firstPage.total,
        pageCount: firstPage.pageCount,
    ...
    async function writeSummary(creator, firstPage, ipLocation, streamRevenue, liverGuild) {
      const summary = summaryValue(creator, firstPage, ipLocation, streamRevenue, liverGuild, now());
    ...
    async function getSummary(context, {force = false} = {}) {
      ...
      return coalesce(inFlight, `summary-network:${uid}`, async () => {
        ...
        const [firstPage, ipLocation, streamRevenue, liverGuild] = await Promise.all([
          fetchGuardPage(creator, 1),
          fetchIpLocation(creator.uid),
          fetchStreamRevenue(creator.uid),
          fetchLiverGuild(creator.uid),
        ]);
        const summary = await writeSummary(creator, firstPage, ipLocation, streamRevenue, liverGuild);
...
(() => {
  ...
  function createPanel({
    ...
    const creatorGuild = textElement(document, 'div', 'guard-creator-guild', '');
    creatorGuild.hidden = true;
    ...
    creator.append(creatorName, creatorUid, creatorLocation, creatorRevenue, creatorGuild, revenueVerification);
    ...
    function clearCreatorRevenue() {
      ...
      creatorRevenue.replaceChildren();
    ...
    function clearCreatorGuild() {
      creatorGuild.hidden = true;
      creatorGuild.className = 'guard-creator-guild';
      creatorGuild.textContent = '';
    }

    function renderCreatorGuild(value) {
      clearCreatorGuild();
      if (value?.status === 'found' && typeof value.name === 'string') {
        const name = value.name.trim().slice(0, 120);
        if (name) {
          creatorGuild.textContent = `所在公会：${name}`;
          creatorGuild.className = 'guard-creator-guild is-found';
          creatorGuild.hidden = false;
          return;
        }
      }
      const status = value?.status === 'no_data' ? 'no_data' : 'unavailable';
      creatorGuild.textContent = status === 'no_data' ? '所在公会：暂无公开信息' : '所在公会：暂不可用';
      creatorGuild.className = `guard-creator-guild is-${status.replaceAll('_', '-')}`;
      creatorGuild.hidden = false;
    }
    ...
    function formatRevenue(total) {
      return Math.trunc(total).toLocaleString('zh-CN');
    }
    ...
    function renderCreatorRevenue(value) {
      ...
      if (value?.status === 'found') {
        ...
        if (
          ...
          creatorRevenue.replaceChildren(
            textElement(document, 'span', 'guard-creator-revenue-line', `礼物/SC流水：${formatRevenue(giftSc)}`),
            textElement(document, 'span', 'guard-creator-revenue-line', `大航海流水：${formatRevenue(guard)}`),
            textElement(document, 'span', 'guard-creator-revenue-line', `充电流水：${formatRevenue(charge)}`),
          );
          ...
          if (url) {
            ...
            creatorRevenue.setAttribute('aria-label', '打开近30日流水详情');
            creatorRevenue.setAttribute('title', '打开近30日流水详情');
          }
    ...
    function renderSummary(summary) {
      ...
      renderCreatorGuild(summary.liverGuild);
...
(() => {
  ...
  const LIVER_GUILD_STATUSES = new Set(['no_data', 'unavailable']);
  ...
  function normalizeLiverGuild(value) {
    ...
    if (value.status === 'found') {
      ...
      return name ? {status: 'found', name} : {status: 'unavailable', name: ''};
    }
    if (LIVER_GUILD_STATUSES.has(value.status)) return {status: value.status, name: ''};
    return {status: 'unavailable', name: ''};
  }
  ...
  function normalizeSummary(value) {
    ...
    return {
      ...
      liverGuild: normalizeLiverGuild(value.liverGuild),
    };
  }
...
(() => {
  ...
  const LIVER_GUILD_PATH_PREFIX = '/https:' + '//' + 'workers.vrp.moe/laplace/bilibili-user/';
  ...
  function createFetch({request, allowedHosts}) {
    ...
    function isAllowedUrl(url) {
      ...
      if (url.hostname === 'fetch.danmakus.com') {
        const uid = url.pathname.startsWith(LIVER_GUILD_PATH_PREFIX)
          ? url.pathname.slice(LIVER_GUILD_PATH_PREFIX.length)
          : '';
        return (
          url.search === ''
          && url.hash === ''
          && /^[1-9][0-9]{0,19}$/u.test(uid)
        );
      }
      return true;
    }
...
(() => {
  ...
  function normalizePayload(payload) {
    ...
    const guard = safeNumber(data.GuardSum ?? data.guardSum);
    const charge = chargeTotal(data.Charge ?? data.charge);
    if (guard === null || charge === null || guard > amount || amount + charge > MAX_REVENUE) {
      return {status: 'unavailable', total: null, giftSc: null, guard: null, charge: null};
    }
    return {status: 'found', total: amount + charge, giftSc: amount - guard, guard, charge};
  }
  ...
  function normalizeCachedEntry(value, uid, now) {
    ...
    if (value.status === 'found') {
      ...
      const giftSc = safeNumber(value.giftSc);
      const guard = safeNumber(value.guard);
      const charge = safeNumber(value.charge);
      if (
        total !== null
        && giftSc !== null
        && guard !== null
...
(() => {
  ...
  async function start(dependencies) {
    ...
    const apiFetch = runtime.userscriptHttp.createFetch({
      ...
      allowedHosts: ['api.bilibili.com', 'api.live.bilibili.com', 'api.gscn.live', 'api.vtb.cat', 'fetch.danmakus.com'],
    });
