...
(() => {
  ...
  const ENDPOINTS = Object.freeze({
    ...
    streamRevenue: 'https://vtb.cat/api/liver/space',
  });
...
(() => {
  ...
  function createPanel({
    ...
    const creatorRevenue = textElement(document, 'div', 'guard-creator-revenue', '');
...
(() => {
  ...
  function createFetch({request, allowedHosts}) {
    ...
    function isAllowedUrl(url) {
      ...
      if (url.hostname === 'vtb.cat') {
        return url.pathname === '/api/liver/space';
      }
      return true;
    }
...
(() => {
  ...
  function create({getValue, setValue, now = Date.now}) {
    ...
    async function capture({location, pageFetch, PageAbortController = globalThis.AbortController}) {
      const uid = displayPageUid(location);
      if (!uid || typeof pageFetch !== 'function') return null;
      const url = `https://vtb.cat/api/liver/space?uid=${encodeURIComponent(uid)}`;
...
(() => {
  ...
  async function start(dependencies) {
    ...
    const apiFetch = runtime.userscriptHttp.createFetch({
      ...
      allowedHosts: ['api.bilibili.com', 'api.live.bilibili.com', 'api.gscn.live', 'vtb.cat'],
    });
  ...
  if (
    ...
    const pageFetch = globalThis.unsafeWindow?.fetch;
    if (typeof pageFetch === 'function' && runtime.userscriptVerifiedRevenue.displayPageUid(globalThis.location)) {
      void verifiedRevenue.capture({
        location: globalThis.location,
        pageFetch: pageFetch.bind(globalThis.unsafeWindow),
        PageAbortController: globalThis.unsafeWindow.AbortController,
      });
      return;
    }
  }
