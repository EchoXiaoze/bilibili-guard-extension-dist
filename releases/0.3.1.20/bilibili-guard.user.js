...
(() => {
  ...
  const ENDPOINTS = Object.freeze({
    ...
    streamRevenue: 'https://api.vtb.cat/api/liver/space',
  });
  ...
  function createBilibiliApi({fetchImpl, sleep, timeoutMs = 8000}) {
    ...
    async function fetchStreamRevenue(uidValue) {
...
(() => {
  ...
  function createGuardBroker({api, storageArea, now}) {
    ...
    function normalizeStreamRevenue(value) {
      ...
      if (value.status === 'no_data') return {status: 'no_data', total: null};
      return {status: 'unavailable', total: null};
    }

    async function fetchStreamRevenue(uid) {
      const key = `streamRevenue:${uid}`;
      const cached = await sessionCache.read(key);
      if (cached.state === 'fresh') return cached.value;
      return coalesce(inFlight, `stream-revenue:${uid}`, async () => {
        const shared = await sessionCache.read(key);
        if (shared.state === 'fresh') return shared.value;
        let revenue;
        try {
          revenue = normalizeStreamRevenue(await api.fetchStreamRevenue(uid));
        } catch {
...
(() => {
  ...
  function createPanel({
    ...
    creator.append(creatorName, creatorUid, creatorLocation, creatorRevenue);
...
(() => {
  ...
  const STREAM_REVENUE_STATUSES = new Set(['no_data', 'unavailable']);
...
(() => {
  const runtime = globalThis.__BILIBILI_GUARD__;
  if (!runtime?.api || !runtime.broker || !runtime.content || !runtime.panelView || !runtime.anchorSign) {
