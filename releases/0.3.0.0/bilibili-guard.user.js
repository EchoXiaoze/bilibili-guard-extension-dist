...
(() => {
  ...
  function createGuardBroker({api, storageArea, now}) {
    if (!api?.resolveVideo || !api?.resolveRoom || !api?.fetchGuardPage) {
    ...
    function summaryValue(creator, firstPage, fetchedAt) {
      return {
        creator,
        total: firstPage.total,
        pageCount: firstPage.pageCount,
    ...
    async function writeSummary(creator, firstPage) {
      const summary = summaryValue(creator, firstPage, now());
    ...
    async function getSummary(context, {force = false} = {}) {
      ...
      return coalesce(inFlight, `summary-network:${uid}`, async () => {
        ...
        const firstPage = await fetchGuardPage(creator, 1);
        const summary = await writeSummary(creator, firstPage);
  ...
  function createController({
    ...
    function ensurePanel() {
      ...
      if (!updateStatusRequested) {
        ...
        Promise.resolve(getUpdateStatus()).then(response => {
...
(() => {
  ...
  function createFetch({request, allowedHosts}) {
    ...
    return async function fetchImpl(input, options = {}) {
      ...
      if (requested.protocol !== 'https:' || !hosts.has(requested.hostname)) throw requestError();
      ...
      return new Promise((resolve, reject) => {
        ...
        const succeed = finish(response => {
          ...
          if (finalUrl.protocol !== 'https:' || !hosts.has(finalUrl.hostname)) {
            reject(requestError());
            return;
          }
...
(() => {
  ...
  function create({requestJson, currentVersion, openInTab, registerMenuCommand, publicBaseUrl}) {
    if (
    ...
    async function check({open = true} = {}) {
      try {
        ...
        return Object.freeze({available, version: manifest.version, url: manifest.userscriptUrl});
      } catch {
...
(() => {
  ...
  async function start(dependencies) {
    ...
    const apiFetch = runtime.userscriptHttp.createFetch({
      ...
      allowedHosts: ['api.bilibili.com', 'api.live.bilibili.com'],
    });
